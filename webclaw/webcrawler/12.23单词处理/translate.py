...
def translate(file_name):
    ...
    for i in tqdm(file):
        # print(i)
        count += 1
        content.append(i+"换行换行")
        # print(content)
        if count % 50 == 0:
            z = content
            content = []
            z = str(z).replace("[", "").replace("]", "").replace("'", "") \
                # .replace(", ", ";    ")
            try:
                title = translate_weapon_BaiduAPI(z)
            except:
                title = "空行空行"
                print(count, "False")
            # print(title)
    ...
    with open('translate_result/实体翻译（1.0）.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=4, ensure_ascii=False)
    f.close()


if __name__ == '__main__':
    translate('cmano_tilte去重（26243）.json')
    pass
