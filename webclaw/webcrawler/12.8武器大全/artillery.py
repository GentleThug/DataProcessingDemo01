...
if __name__ == '__main__':
    with open('网页/artillery.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    tiaoshu = 1
    all = []
    for d in data:
        ...
        tiaoshu += 1
        # if tiaoshu % 500 == 0:
        #     with open('武器类别/artillery' + str(tiaoshu) + '.json', 'w', encoding='utf-8') as f:
        #         json.dump(all, f, indent=4, ensure_ascii=False)

    with open('武器类别/artillery' + str(len(all)) + '.json', 'w', encoding='utf-8') as f:
        json.dump(all, f, indent=4, ensure_ascii=False)
