        ...
        if cdiv_local <= cdiv_num:
            ...
            # 技术数据cdiv
                ...
                for i in t.find_all("li"):
                    ...
                    if str_title == "规格" or str_title == "有效载荷" or str_title == "结构尺寸":
...
if __name__ == '__main__':
    with open('网页/spaceship.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    print(data)
    tiaoshu = 1
    all = []
    for d in data:
        ...
        tiaoshu += 1
        # if tiaoshu % 500 == 0:
        #     with open('武器类别/spaceship' + str(tiaoshu) + '.json', 'w', encoding='utf-8') as f:
        #         json.dump(all, f, indent=4, ensure_ascii=False)

    with open('武器类别/spaceship' + str(len(all)) + '.json', 'w', encoding='utf-8') as f:
        json.dump(all, f, indent=4, ensure_ascii=False)
