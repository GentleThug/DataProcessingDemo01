...
def pachong(num):
    global response
    url = 'http://cmano-db.com/sensor/' + str(num) + '/'
    headers = get_headers()
    try:
...
if __name__ == '__main__':
    ...
    for i in range(chushi):
        ...
        if result:
            ...
            if tiaoshu % 1000 == 0:
                with open('12.9/sensor' + str(tiaoshu) + '.json', 'w', encoding='utf-8') as f:
    ...
    with open('12.9/sensor' + '全部' + str(tiaoshu) + '.json', 'w', encoding='utf-8') as f:
        json.dump(all, f, indent=4, ensure_ascii=False)
